...
mkdir -p /root/.config
ln -sfn /data /root/.config/usernamebot
...
if base_url:
    try:
        import database
        database.init_db()
        database.set_config("base_url", base_url)
    except Exception as e:
        pass

invite_base_url = options.get("invite_base_url", "").strip()
if invite_base_url:
    try:
        import database
        database.init_db()
        database.set_config("invite_base_url", invite_base_url)
    except Exception as e:
        pass

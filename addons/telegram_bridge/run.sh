...
echo "============================================================"
echo " Starting Delta Chat Telegram Bridge..."
echo "============================================================"
...
tg_token = options.get("telegram_token", "").strip()
if tg_token:
    os.environ["TELEGRAM_TOKEN"] = tg_token
    try:
        import database
        database.init_db()
        database.set_config("telegram_token", tg_token)
    except Exception as e:
        print(f"Setting telegram_token note: {e}")
